...
//				rc.getType().attackRadiusSquared, rc.getTeam().opponent());
//
//		if (nearbyEnemies.length > 0) { // there exists
//																// enemies near
//			// find the first tower and shoot at it
//			for (RobotInfo info : nearbyEnemies) {
//				if (info.type.equals(RobotType.TOWER)) {
...
//				}
//			}
//			if (Clock.getRoundNum()>1600 && nearbyTower != null && rc.canLaunch(rc.getLocation().directionTo(nearbyTower))) {
//				rc.launchMissile(rc.getLocation().directionTo(nearbyTower));
//				System.out.println("launchingtower");
//			} else {
//				missileLowestHP(nearbyEnemies);
//			}
//		}
	
	}

	private static void missileLowestHP(RobotInfo[] enemies) throws GameActionException {
		if (enemies.length == 0) {
			return;
		}
